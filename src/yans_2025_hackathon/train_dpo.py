...
def _count_characters_in_dataset(dataset: Dataset) -> int:
    num_chars = 0
    for item in dataset:
        for message in item["chosen"] + item["rejected"]:
...
def train_dpo(
    ...
    Raises:
        ValueError: データセットに"chosen"または"rejected"フィールドがない場合
        ValueError: サンプル数が500を超える場合
        ValueError: 総文字数が50万文字を超える場合
    """
    required_fields = ["chosen", "rejected"]
    for field in required_fields:
        if field not in train_dataset.features:
            msg = f"`train_dataset` は `{field}` フィールドを含む必要があります。"
